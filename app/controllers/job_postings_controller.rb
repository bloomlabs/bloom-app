class JobPostingsController < ApplicationController
  ...
  def index
    @job_postings = JobPosting.where('expiry <= ? and closed = false', Date.today)
  end
  ...
  def authenticate_editor
    if not @job_posting.allows_editing(current_user)
      render 404
    end
  end
  ...
  def job_posting_params
    params.require(:job_posting).permit(:description, :closed, :title, :expiry, :requirements)
  end
end
